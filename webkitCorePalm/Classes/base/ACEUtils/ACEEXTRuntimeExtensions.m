...
typedef void (^ace_specialProtocolInjectionBlock)(Class);

// a `const char *` equivalent to system struct objc_method_description
typedef struct {
    SEL name;
    const char *types;
} ace_methodDescription;
 ...
 */
static void ace_injectSpecialProtocols (void) {
    /*
    ...
    // use this instead of ace_copyClassList() to avoid sending +initialize to
    ...
    @autoreleasepool {
        ...
        for (size_t i = 0;i < specialProtocolCount;++i) {
            ...
            // from the structure
            ace_specialProtocolInjectionBlock injectionBlock = (__bridge_transfer id)specialProtocols[i].injectionBlock;
...
unsigned ace_injectMethods (
                            Class aClass,
                            Method *methods,
                            unsigned count,
                            ace_methodInjectionBehavior behavior,
                            ace_failedMethodCallback failedToAddCallback
                            ) {
    ...
    @autoreleasepool {
        ...
        if (!isMeta) {
            ...
            behavior &= ~(ace_methodInjectionIgnoreLoad | ace_methodInjectionIgnoreInitialize);
        }
        ...
        for (unsigned methodIndex = 0;methodIndex < count;++methodIndex) {
            ...
            if (behavior & ace_methodInjectionIgnoreLoad) {
            ...
            if (behavior & ace_methodInjectionIgnoreInitialize) {
            ...
            switch (behavior & ace_methodInjectionOverwriteBehaviorMask) {
                case ace_methodInjectionFailOnExisting:
                    success = class_addMethod(aClass, methodName, impl, type);
                    break;
                    
                case ace_methodInjectionFailOnAnyExisting:
                ...
                case ace_methodInjectionReplace:
                    class_replaceMethod(aClass, methodName, impl, type);
                    break;
                    
                case ace_methodInjectionFailOnSuperclassExisting:
                {
...
BOOL ace_injectMethodsFromClass (
                                 Class srcClass,
                                 Class dstClass,
                                 ace_methodInjectionBehavior behavior,
                                 ace_failedMethodCallback failedToAddCallback)
{
    ...
    // ignore +load
    behavior |= ace_methodInjectionIgnoreLoad;
    addedCount = ace_injectMethods(
...
Class ace_classBeforeSuperclass (Class receiver, Class superclass) {
...
Class *ace_copyClassList (unsigned *count) {
    ...
    @autoreleasepool {
        ...
        for (int i = 0;i < classCount;) {
            ...
            if (keep) {
                ...
                if (class_respondsToSelector(class, @selector(isProxy)) && class != NSClassFromString(@"CLTilesManagerClient")){
                    keep = ![class isProxy];
                }
            }
...
unsigned ace_addMethods (Class aClass, Method *methods, unsigned count, BOOL checkSuperclasses, ace_failedMethodCallback failedToAddCallback) {
    ace_methodInjectionBehavior behavior = ace_methodInjectionFailOnExisting;
    if (checkSuperclasses)
        behavior |= ace_methodInjectionFailOnSuperclassExisting;
    
    return ace_injectMethods(
                             aClass,
                             methods,
                             count,
                             behavior,
...
BOOL ace_addMethodsFromClass (Class srcClass, Class dstClass, BOOL checkSuperclasses, ace_failedMethodCallback failedToAddCallback) {
    ace_methodInjectionBehavior behavior = ace_methodInjectionFailOnExisting;
    if (checkSuperclasses)
        behavior |= ace_methodInjectionFailOnSuperclassExisting;
    
    return ace_injectMethodsFromClass(srcClass, dstClass, behavior, failedToAddCallback);
}

BOOL ace_classIsKindOfClass (Class receiver, Class aClass) {
    while (receiver) {
        if (receiver == aClass)
            return YES;
...
Class *ace_copyClassListConformingToProtocol (Protocol *protocol, unsigned *count) {
    Class *allClasses;
    ...
    @autoreleasepool {
        ...
        allClasses = ace_copyClassList(&classCount);
...
ace_propertyAttributes *ace_copyPropertyAttributes (objc_property_t property) {
    ...
    ace_propertyAttributes *attributes = calloc(1, sizeof(ace_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate ace_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        switch (flag) {
            ...
            case 'C':
                attributes->memoryManagementPolicy = ace_propertyMemoryManagementPolicyCopy;
                break;
                
            case '&':
                attributes->memoryManagementPolicy = ace_propertyMemoryManagementPolicyRetain;
                break;
...
Class *ace_copySubclassList (Class targetClass, unsigned *subclassCount) {
    unsigned classCount = 0;
    Class *allClasses = ace_copyClassList(&classCount);
...
Method ace_getImmediateInstanceMethod (Class aClass, SEL aSelector) {
...
BOOL ace_getPropertyAccessorsForClass (objc_property_t property, Class aClass, Method *getter, Method *setter) {
    ace_propertyAttributes *attributes = ace_copyPropertyAttributes(property);
    if (!attributes)
        return NO;
...
NSMethodSignature *ace_globalMethodSignatureForSelector (SEL aSelector) {
    ...
    static ace_methodDescription volatile methodDescriptionCache[selectorCacheLength];
    ...
    ace_methodDescription methodDesc;
    ...
    methodDesc = (ace_methodDescription){.name = NULL, .types = NULL};
    
    uint classCount = 0;
    Class *classes = ace_copyClassList(&classCount);
    if (classes) {
        @autoreleasepool {
            ...
            for (uint i = 0;i < classCount;++i) {
                ...
                if (method) {
                    methodDesc = (ace_methodDescription){.name = aSelector, .types = method_getTypeEncoding(method)};
                    break;
                }
            }
        }
    ...
    if (!methodDesc.name) {
        ...
        if (protocols) {
            ...
            for (uint i = 0;i < protocolCount;++i) {
                ...
                if (objcMethodDesc.name) {
                    methodDesc = (ace_methodDescription){.name = objcMethodDesc.name, .types = objcMethodDesc.types};
                    break;
                }
            }
            free(protocols);
...
BOOL ace_loadSpecialProtocol (Protocol *protocol, void (^injectionBehavior)(Class destinationClass)) {
    @autoreleasepool {
        ...
        // ace_injectSpecialProtocols()
#ifndef __clang_analyzer__
        ace_specialProtocolInjectionBlock copiedBlock = [injectionBehavior copy];
...
void ace_specialProtocolReadyForInjection (Protocol *protocol) {
    @autoreleasepool {
        NSCParameterAssert(protocol != nil);
...
void ace_removeMethod (Class aClass, SEL methodName) {
    Method existingMethod = ace_getImmediateInstanceMethod(aClass, methodName);
    if (!existingMethod) {
        return;
    }
...
void ace_replaceMethods (Class aClass, Method *methods, unsigned count) {
    ace_injectMethods(
                      aClass,
                      methods,
                      count,
                      ace_methodInjectionReplace,
                      NULL
                      );
}

void ace_replaceMethodsFromClass (Class srcClass, Class dstClass) {
    ace_injectMethodsFromClass(srcClass, dstClass, ace_methodInjectionReplace, NULL);
}

NSString *ace_stringFromTypedBytes (const void *bytes, const char *encoding) {
    switch (*encoding) {
