...
@interface PullDownRefreshHeaderView (Private)
- (void)setState:(PullRefreshState)aState;
@end

@implementation PullDownRefreshHeaderView
...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
		...
		layer.contents = (id)[UIImage imageNamed:@"img/blueArrow.png"].CGImage;
...
- (void)refreshLastUpdatedDate {
	...
	if ([_delegate respondsToSelector:@selector(refreshHeaderDataSourceLastUpdated:)]) {
		...
		_lastUpdatedLabel.text = [NSString stringWithFormat:@"%@: %@",ACELocalized(@"上次更新时间"), [formatter stringFromDate:date]];
		[[NSUserDefaults standardUserDefaults] setObject:_lastUpdatedLabel.text forKey:@"refreshView_LastRefresh"];
...
- (void)setState:(PullRefreshState)aState{
	...
	switch (aState) {
		case PullRefreshPulling:
            ...
            _statusLabel.text = ACELocalized(@"释放刷新");
			[CATransaction begin];
...
- (void)refreshScrollViewDidScroll:(UIScrollView *)scrollView {	
	...
	if (_state == PullRefreshLoading) {
		...
		BOOL _loading = NO;
		if ([_delegate respondsToSelector:@selector(refreshHeaderDataSourceIsLoading:)]) {
			_loading = [_delegate refreshHeaderDataSourceIsLoading:self];
		}
